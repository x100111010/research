...
import plyvel
import dbobjects_pb2 as SpectredDB
from tqdm.auto import tqdm

# A bunch of Spectre DB kay and store names used below
sep = b"/"
level = (0).to_bytes(
    1, "little"
...
class HeaderData:
    ...
    def __init__(self, db_header):
        """
        :param db_header:
                SpectredDB.DbBlockHeader fields:
...
class BlockData:
    ...
    def __init__(self, db_block):
        ...
        self.max_sig_ops = max(
            max((inp.sigOpCount for inp in t.inputs), default=0)
            for t in db_block.transactions
        )
        self.sum_sig_ops = sum(
            sum((inp.sigOpCount for inp in t.inputs)) for t in db_block.transactions
        )
        ...
        if len(payload) > pubkey_end:
            try:
                ...
                if "/" in extra_data:
                    ...
                    self.spectred_version = extra_data[:index_of_sep]
                    self.miner_version = extra_data[index_of_sep + 1 :]
                else:
                    self.spectred_version = extra_data
            except:
                pass
...
class Store:
    """
    Class managing all accesses to the underlying Spectre DB
    """
    ...
    def get_chain_block_hash_by_index(self, index):
        hash_bytes = self.db.get(
            self.prefix
            + sep
            + chain_block_hash_by_index
            + sep
    ...
    def get_chain_block_index_by_hash(self, block_hash):
        index_bytes = self.db.get(
            self.prefix + sep + chain_block_index_by_hash + sep + block_hash
        )
        index = int.from_bytes(index_bytes, "little")
        return index
    ...
    def get_block_status(self, block_hash):
        status_bytes = self.db.get(
            self.prefix + sep + block_status_store + sep + block_hash
        )
        if status_bytes is None:
            return None
        s = SpectredDB.DbBlockStatus()
    ...
    def get_ghostdag_data(self, block_hash):
        ghostdag_data_bucket = (
            self.prefix + sep + level + sep + ghostdag_data_store + sep
        )
        ghostdag_data_bytes = self.db.get(ghostdag_data_bucket + block_hash)
        gdd = SpectredDB.DbBlockGhostdagData()
    ...
    def get_detailed_ghostdag_data(self, block_hash):
        ...
        gdd.ParseFromString(ghostdag_data_bytes)
        return (
            [b.hash for b in gdd.mergeSetBlues],
            [r.hash for r in gdd.mergeSetReds],
            gdd.selectedParent.hash,
        )
    ...
    def get_block(self, block_hash):
        ...
        if block_relations_bytes is None:
            return None
        br = SpectredDB.DbBlockRelations()
    ...
    def get_virtual_reds(self, threshold=0, time_distance=0):
        ...
        current = hst
        while current != pp:
            mergeset_blues, mergeset_reds, selected_parent = (
                self.get_detailed_ghostdag_data(current)
            )
    ...
    def load_utxo_data(self, fields=None):
        ...
        for key, value in tqdm(
            self.db.iterator(prefix=self.prefix + sep + virtual_utxo_set_key)
        ):
            db_entry = SpectredDB.DbUtxoEntry()
    ...
    def pruning_point(self):
        ...
        pp_bytes = self.db.get(
            self.prefix
            + sep
            + pruning_by_index_store
            + sep
            + pp_index.to_bytes(8, "big")
        )
        pp = SpectredDB.DbHash()
